...
let spin = function(io, user){
	...
	let nohu    = false;
	// tạo kết quả
	HU.findOne({game:'tamhung', type:bet, red:true}, {}, function(err2, dataHu){
				...
				if(line_win.win == 5) {
					if (line_win.type === 5) {
						...
						if (!nohu) {
							...
							bet_win += okHu;
							HU.updateOne({game:'tamhung', type:bet, red:true}, {$set:{name:'', bet:dataHu.min}}).exec();
						}else{
...
/* 			if (!nohu && bet_win >= tongCuoc*2.24) {
				io.sendInHome({news:{t:{game:'VƯƠNG QUỐC ZON', users:user.name, bet:bet_win, status:2}}});
			} */
			HU.updateOne({game:'tamhung', type:bet, red:true}, {$inc:huUpdate}).exec();
...
module.exports = function(io, listBot){
	if (listBot != null && listBot.length) {
