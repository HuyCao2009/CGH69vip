...
module.exports = function(client, data){
	if (!!data.captcha && !!data.code) {
		...
		}else if (!validator.isLength(data.code, {min: 4, max: 12})) {
			console.log("vao>>>>>>>>", data.code);
							...
							if (null !== check.uid) {
								...
								if (validator.isEmpty(check.type)) {
									...
									UserInfo.findOneAndUpdate({id: client.UID}, {$inc:{red:check.red, xu:check.xu}}).exec(function(err, user){
										client.red({notice:{title:'THÀNH CÔNG',text:'Bạn nhận được: ' + (check.red > 0 ? Helpers.numberWithCommas(check.red) + ' XU' : '') /*+ (check.xu > 0 ? (check.red > 0 ? ' và ' : '') + Helpers.numberWithCommas(check.xu) + ' XU' : '')*/}, user:{red:user.red*1+check.red, xu:user.xu*1+check.xu}});
									});
								}else{
