...
let Zeus_xu   = require('../../../Models/Zeus/Zeus_xu');
...
module.exports = function(client, data){
	...
	if (!!data && !!data.cuoc && Array.isArray(data.line)) {
		...
		if (!(bet == 100 || bet == 1000 || bet == 10000) || line.length < 1) {
			...
			//loadCached(bet);
			client.Zeus = void 0 === client.Zeus ? {id:'', red:red, bet:bet, bonus:null, bonusL:0, bonusWin:0, free:0} :client.Zeus;
			client.Zeus.red = red;
			client.Zeus.bet = bet;
			let tongCuoc = bet*line.length;
			UserInfo.findOne({id:client.UID}, red ? 'red name':'xu name', function(err, user){
				if (client.Zeus.free === 0 && ((red && user.red < tongCuoc) || (!red && user.xu < tongCuoc))) {
					client.red({Zeus:{status:0, notice:'Bạn không đủ ' + (red ? 'XU':'XU') + ' để quay.!!'}});
				}else{
					let config = require('../../../../config/Zeus.json');
					let phe = red ? 2 :4;    // Phế
					...
					let isBigWin  = false;
					// tạo kết quả
					HU.findOne({game:'Zeus', type:bet, red:red}, {}, function(err2, dataHu){
						let uInfo      = {};
						let mini_users = {};
						...
						.then(result => {
								...
								if(!nohu && line_win.win == 10) {
									...
									if (line_win.type === 5) {
										...
										if (!nohu) {
											...
											bet_win += okHu;
											HU.updateOne({game:'Zeus', type:bet, red:red}, {$set:{name:'', bet:dataHu.min}}).exec();
							...
							if (!nohu && bet_win >= tongCuoc*2.24) {
								...
								type = 1;
								if (red) {
									client.redT.sendInHome({news:{t:{game:'Zeus', users:client.profile.name, bet:bet_win, status:2}}});
								}
							}
							if (free > 0) {
							...
							if (red) {
								...
								Zeus_red.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
								...
								Zeus_xu.create({'name':client.profile.name, 'type':type, 'win':bet_win, 'bet':bet, 'kq':result.length, 'line':line.length, 'time':new Date()}, function (err4, small) {
									client.red({zeus:{status:1, cel:[cel1, cel2, cel3, cel4, cel5], line_win:result, win:bet_win, free:client.zeus.free, isFree:isFree, isBonus:!!client.zeus.bonusX, isNoHu:nohu, isBigWin:isBigWin, thuong:thuong, phien:small.id}, user:{xu:user.xu-tongCuoc}});
									client.zeus.id = small._id.toString();
								});
							}
							HU.updateOne({game:'Zeus', type:bet, red:red}, {$inc:huUpdate}).exec();
