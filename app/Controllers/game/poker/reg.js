					...
					if (inGame) {
						...
						UserInfo.findOne({id: client.UID}, 'red xu name', function(err, user){
							if (!user || (red && user.red < min) || (!red && user.xu < min)) {
								client.red({notice:{title:'THẤT BẠI', text:'Bạn cần tối thiểu ' + Helpers.numberWithCommas(min) + (red ? ' XU':' XU') + ' để vào phòng.!!', load: false}});
							}else{
								if ((red && user.red < balans) || (!red && user.xu < balans)) {
									...
									client.red({notice:{title:'THẤT BẠI', text:'Bạn chỉ có thể mang tối đa ' + Helpers.numberWithCommas(minMang) + (red ? ' XU':' XU') + ' vào phòng chơi.!!', load: false}});
								}else{
									/**
									if (red){
										user.red -= balans;
