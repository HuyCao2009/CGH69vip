		...
		if (!(cuoc == 100 || cuoc == 1000 || cuoc == 10000)) {
			...
			UserInfo.findOne({id:client.UID}, 'red xu name', function(err, user){
				if (!user || (red && user.red < cuoc) || (!red && user.xu < cuoc)) {
					client.red({mini:{bacay:{status:0, notice: 'Bạn không đủ ' + (red ? 'XU':'XU') + ' để quay.!!'}}});
				}else{
					let phe = red ? 2 : 4;    // Phế
					...
					HU.findOne({game:'mini3cay', type:cuoc, red:red}, {}, function(err, data){
						...
						if (red){
							...
							Mini3Cay_red.create({'uid': client.UID, 'win': an, 'bet': cuoc, 'type': code, 'kq': ketqua, 'time': new Date(), name: client.profile.name});
